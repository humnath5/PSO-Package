...
double fit_noghost(double , int);
double poten_energy(double *, double, double, double);
...
char bounds_file[40];
strcpy(bounds_file,"bounds_noghost.txt");
...
approxV[i]= poten_energy(gbest,rH1I[i],rH2I[i],rOI[i]);
}
//fprintf(fp,"%lf\t%lf\n",V[i],approxV[i]);}
...
if(myrank == 0){
FILE* fp;
fp=fopen("output_noghost.txt","w");

fprintf(fp,"\n**================== PSO Parameters Used ==================**\n");
fprintf(fp,"seed1:%d\n",seed1);
fprintf(fp,"seed2:%d\n",seed2);
frintf("Number of processors used: %d\n",comsize);
...
for(i= 0;i< numofgeom;i++){
 ...
 fprintf(fp,"%lf\t%lf\t%lf\t%lf\t%lf\n",rH1I[i],rH2I[i],rOI[i],V[i],approxV[i]);
}
fclose(fp);
}
MPI_Finalize();
...
double fit_noghost(double * x, int nd){// nd is not necessary in this function but kept to be consistent for PSO input
  int i;
     double rmse = 0;
     double sse = 0;

     for( i= 0; i< numofgeom; i++) {
          approxV[i] = poten_energy(x,rH1I[i],rH2I[i],rOI[i]);
...
double poten_energy(double * x, double r1, double r2, double r3){

     // x -> parameter vector to be optimized

    double  m1,m2; // integer parameters
    double  n1,n2; //integer parameters
...
if (strcmp(fName,"fit_noghost")==0){return &fit_noghost;}
