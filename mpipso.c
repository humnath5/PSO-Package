...
void mpipso_optimization(fptr fun, int np , int nd, int ni,double* lb, double* ub, double* value, double* gbest, int ni_stop, double tol, int myrank){
...
void mpimulti_pso_optimization(fptr fun, int np , int nd, int ni, int maxRun, int* type, double* lb, double* ub, double* value ,double* gbest, int ni_stop, double tol, int myrank)
{   int i;
    double alpha = 1;
    for( i = 0 ; i < maxRun ; i++) {
...
void mpimulti_pso_local_optimizer(fptr fun, int np, int nd, int ni, int maxRun, int* type, double* lb, double* ub, double* value, double* gbest, int ni_stop, double tol, int myrank)
{    int i;
     ...
     for( i = 0 ; i < maxRun ; i++) {
       ...
       mpipso_optimization(FUN, np , ND, ni, lb, ub, value, GBEST, ni_stop,tol, myrank);
       //printf("Value after %d run  %f\n", i, (*value));
       local_optimizer(FUN, ND, lb, ub, value, GBEST, ni_stop, tol,myrank);
...
void  local_optimizer(fptr fun,int nd, double* lb, double* ub, double* value, double* gbest, int ni_stop, double tol, int myrank) {
		...
		double best = fun(gbest, nd);
		double alpha = (double)(myrank/(myrank+1));
